...
"""
Daisy Fiscal Printer Detection Plugin

Implements auto-detection for Daisy fiscal printers using ISL protocol.
"""
...
@register_plugin
class DaisyDetectionPlugin(FiscalPrinterDetectionPlugin):
    """
    Detection plugin for Daisy fiscal printers.
    
    Daisy printers use ISL protocol with:
    - Similar structure to other ISL devices
    - Serial number starts with "DY"
    - Command 0x80 for device constants
    """
    
    PLUGIN_NAME = "daisy.isl"
    MANUFACTURER = "Daisy"
    SUPPORTED_MODELS = [
        "Daisy CompactM", "Daisy CompactS",
        "Daisy eXpert", "Daisy FP3530"
    ]
    ...
    def get_priority(self) -> int:
        """Daisy is moderately common in Bulgaria"""
        return 25
    
    def probe_device(self, connection, baudrate: int = 115200) -> Tuple[DetectionResult, Optional[str]]:
        """
        Probe for Daisy device using device constants command.
        
        Sends device constants command and checks for Daisy-specific response.
        """
        try:
            # Clear buffers
            ...
            if response[0:1] == bytes([self.STX]):
                # Verify it's Daisy by checking serial number prefix
                data_str = response.decode('cp1251', errors='ignore')
                
                if self.SERIAL_NUMBER_PREFIX in data_str:
                    self._logger.debug("Daisy probe successful - found DY prefix")
                    return (DetectionResult.DETECTED, None)
                else:
                    return (DetectionResult.NOT_THIS_DEVICE, "No DY prefix found")
            else:
    ...
    def get_device_info(self, connection, baudrate: int = 115200) -> DeviceIdentification:
        """
        Get detailed Daisy device information.
        ...
        try:
            ...
            if len(parts) < 2:
                raise Exception("Invalid Daisy device info format")
            ...
            raise Exception(f"Failed to get Daisy device info: {e}")
    
    def _build_isl_message(self, cmd: int, data: bytes) -> bytes:
        """
        Build ISL protocol message for Daisy.
    ...
    def validate_serial_number(self, serial: str) -> bool:
        """
        Validate Daisy serial number.
        
        Daisy serial numbers:
