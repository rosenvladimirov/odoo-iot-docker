...
"""
Eltrade Fiscal Printer Detection Plugin

Implements auto-detection for Eltrade fiscal printers using ISL protocol.
"""
...
@register_plugin
class EltradeDetectionPlugin(FiscalPrinterDetectionPlugin):
    """
    Detection plugin for Eltrade fiscal printers.
    
    Eltrade printers use ISL protocol with:
    - Similar structure to other ISL devices
    - Serial number starts with "ED"
    - Command 0x80 for device constants
    """
    
    PLUGIN_NAME = "eltrade.isl"
    MANUFACTURER = "Eltrade"
    SUPPORTED_MODELS = [
        "Eltrade A1", "Eltrade A3",
        "Eltrade A6", "Eltrade FP"
    ]
    ...
    def get_priority(self) -> int:
        """Eltrade is moderately common in Bulgaria"""
        return 30
    
    def probe_device(self, connection, baudrate: int = 115200) -> Tuple[DetectionResult, Optional[str]]:
        """
        Probe for Eltrade device using device constants command.
        
        Sends device constants command and checks for Eltrade-specific response.
        """
        try:
            # Clear buffers
            ...
            if response[0:1] == bytes([self.STX]):
                # Verify it's Eltrade by checking serial number prefix
                data_str = response.decode('cp1251', errors='ignore')
                
                if self.SERIAL_NUMBER_PREFIX in data_str:
                    self._logger.debug("Eltrade probe successful - found ED prefix")
                    return (DetectionResult.DETECTED, None)
                else:
                    return (DetectionResult.NOT_THIS_DEVICE, "No ED prefix found")
            else:
    ...
    def get_device_info(self, connection, baudrate: int = 115200) -> DeviceIdentification:
        """
        Get detailed Eltrade device information.
        ...
        try:
            ...
            if len(parts) < 2:
                raise Exception("Invalid Eltrade device info format")
            ...
            raise Exception(f"Failed to get Eltrade device info: {e}")
    
    def _build_isl_message(self, cmd: int, data: bytes) -> bytes:
        """
        Build ISL protocol message for Eltrade.
    ...
    def validate_serial_number(self, serial: str) -> bool:
        """
        Validate Eltrade serial number.
        
        Eltrade serial numbers:
